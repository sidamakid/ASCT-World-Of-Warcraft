...
f:SetScript("OnEvent", --Run when our event fires
   function(self, event, unit, _, spellName)
    local spellName = GetSpellInfo(spellName)
      if unit == "player" and SpellTableLust[spellName] and SARTESPELLDB["Enhancement"]["Lust"] == true
      then
         ...
         spellFrame:SetScript("OnUpdate", --Run forever!
            function()
               ...
               if start == 0 then
                CombatText_AddMessage("Lust is now ready!", CombatText_StandardScroll, 1, 1, 0);
