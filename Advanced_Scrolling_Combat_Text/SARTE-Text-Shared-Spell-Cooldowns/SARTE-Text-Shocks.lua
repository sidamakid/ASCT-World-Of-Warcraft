local isClassic_TBC_Wrath = (LE_EXPANSION_LEVEL_CURRENT == LE_EXPANSION_CLASSIC) or (LE_EXPANSION_LEVEL_CURRENT == LE_EXPANSION_BURNING_CRUSADE) or (LE_EXPANSION_LEVEL_CURRENT == LE_EXPANSION_WRATH_OF_THE_LICH_KING)
...
f:SetScript("OnEvent", --Run when our event fires
   function(self, event, unit, _, spellName)
      ...
      if unit == "player" then
       if SpellTableShamanShocks[ASCT_GetEnglishName(spellName)] and Advanced_Scrolling_Combat_Text_DB["Shared_Shaman_spells"]["Shocks"] == true then
