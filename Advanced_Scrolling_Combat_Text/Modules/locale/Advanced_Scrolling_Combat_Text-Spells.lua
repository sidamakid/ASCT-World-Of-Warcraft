...
local Locale, Database, Frames, Scripts, API = ASCT.Locale, ASCT.Database, ASCT.Frames, ASCT.Scripts, ASCT.API
local callbacks = {}
...
Locale.Spells["ASCT_LoadSpellNames"] = function ()
  ...
  Scripts.Frame["OnEvent"](loadingFrame, function(self, eventName, spellID, success)
    ...
    if left <= 0 then
      ...
      for key, spellID in pairs(Database.Spells["ASCT_Locale"]) do
        local localeName, _, icon = API.Documentation["GetSpellInfo"](spellID)
        local IsSpellKnwon = API.Documentation["GetSpellInfo"](localeName) ~= nil
        local SpellDescription = API.Documentation["GetSpellDescription"](spellID)
...
Locale.Spells["ASCT_GetLocalizedISpellKnown"] = function(english)
