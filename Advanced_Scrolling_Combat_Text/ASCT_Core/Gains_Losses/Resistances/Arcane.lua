...
local lastStat = -1
local f = Frames.SARTE["Arcane_Resistance_Frame"]
Functions.Initializers["Advanced_Scrolling_Combat_Text_AddInitializer"](function()
Scripts.Frame["OnEvent"](f, function()
    local stats = ASCT_DB["Advanced_Scrolling_Combat_Text_Resistance"]["Arcane Resistance"]
    if not (stats.StatEnable and API.Documentation["C_CVar.GetCVarBool"](Strings.C_CVar["enableFloatingCombatText"])) then return end
    local base, total, bonus, minus = API.Documentation["UnitResistance"](Strings.UnitId["player"], Str_Int.Resistance["Arcane"])
