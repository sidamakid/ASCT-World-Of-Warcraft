...
local lastStat = -1
local f = Frames.SARTE["Nature_Resistance_Frame"]
...
Scripts.Frame["OnEvent"](f, function()
    ...
    local base, total, bonus, minus = API.Documentation["UnitResistance"](Strings.UnitId["player"], Str_Int.Resistance["Nature"])
    ...
    elseif (diff < 0 and stats.Lost) or (diff > 0 and stats.Gains) then
        local msg = string.format("%s%d %s (%d)", (diff>0) and "+" or "", diff, L["Nature Resistance"],  total)
