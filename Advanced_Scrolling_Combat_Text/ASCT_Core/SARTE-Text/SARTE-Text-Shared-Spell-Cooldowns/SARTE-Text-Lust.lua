...
if ASCT.Client["isTbcWowOrAbove"] then
...
---------------------------
local f = ASCT.Frames.SARTE["SARTE-Text-Shared-Spell-Cooldowns_Frame"]
...
ASCT.Scripts.Frame["OnEvent"](f, --Run when our event fires
   function(self, event, unit, _, spellName)
      ...
      if unit == ASCT.Strings.UnitId["player"] then
       if SpellTableShamanLust[ASCT.Locale.Spells["ASCT_GetEnglishName"](spellName)] and ASCT_DB["Shared_Shaman_spells"]["Lust"] == true then
