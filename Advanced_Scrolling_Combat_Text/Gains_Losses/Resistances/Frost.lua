...
f:SetScript("OnEvent", function()
    local stats = Advanced_Scrolling_Combat_Text_DB["Advanced_Scrolling_Combat_Text_Stats"]["Frost Resistance"]
    ...
    elseif (diff < 0 and stats.Lost) or (diff > 0 and stats.Gains) then
        CombatText_AddMessage(format("%s%d %s (%d)", (diff>0) and "+" or "", diff, L["Frost Resistance"],  total), CombatText_StandardScroll, 0.1, 0.1, 1)
    end
    lastStat = total
end)
end)
